		...
		txtFechaSalida.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				if ("date".equals(evt.getPropertyName())) {
				    ...
				    long diDernciaDia = fechaSalida.getTime() - fechaEntrada.getTime();
				    TimeUnit time = TimeUnit.DAYS;
				    long diferencia = time.convert(diDernciaDia, TimeUnit.MILLISECONDS);
				    if(fechaEntrada != null && fechaSalida != null) {
				    	if(diferencia >= 0) {
				    		valor = (diferencia + 1) * 15.0f;
				    		valorReserva = String.valueOf(valor);
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {
				   ...
				   if (fechaEntrada.getTime() < fechaActual.getTime()) {
					   JOptionPane.showMessageDialog(null, "La fecha inicial de la reservacion no puede ser menor a"
					   		+ " la fecha de hoy");
					   txtFechaEntrada.requestFocusInWindow();
					   return;
				   }
				   if(fechaSalida.getTime()  < fechaActual.getTime()) {
					   JOptionPane.showMessageDialog(null, "La fecha final de la reservacion no debe ser menor a la "
					   		+ " fecha actual");
					   txtFechaSalida.requestFocusInWindow();
					   return;
				   }
				   if(fechaSalida.getTime()	< fechaEntrada.getTime()){
					   JOptionPane.showMessageDialog(null, "La fecha final de la reservacion no debe ser menor a la fecha"
					   		+ " inicial de la reservacion");
					   return;
				   }
	...
	//Código que permite mover la ventana por la pantalla según la posición de "x" y "y"	
	 private void headerMousePressed(java.awt.event.MouseEvent evt) {
	    ...
	    private void agregarReserva() {
			...
			Date fechaEntrada = txtFechaEntrada.getDate();
			Date fechaSalida = txtFechaSalida.getDate();
			ModeloReservacion reservacion = new ModeloReservacion(fechaEntrada, fechaSalida, valor, forma_pago);
			
			ReservacionController reservaController = new ReservacionController();
			idREserva = reservaController.guardaReservacion(reservacion);
